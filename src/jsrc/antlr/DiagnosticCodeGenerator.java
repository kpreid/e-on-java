...
public class DiagnosticCodeGenerator extends CodeGenerator {
    ...
    public void gen(LexerGrammar g) throws IOException {
        ...
        currentOutput =
          antlrTool.openOutputFile(grammar.getClassName() + TokenTypesFileExt);
...
//        }
        ...
        if (rr.args != null && rs.block.argAction == null) {
            println(
              "Error: Rule '" + rr.targetRule + "' accepts no arguments.");
        }
    }
    ...
    protected void genAlt(Alternative alt) {
        ...
        if (alt.getTreeSpecifier() != null) {
            println(
              "AST will be built as: " + alt.getTreeSpecifier().getText());
        }
    }

    /**
     * Generate the header for a block, which may be a RuleBlock or a plain
     * AlternativeBLock.  This generates any variable declarations,
    ...
    public void genCommonBlock(AlternativeBlock blk) {
        ...
        if (singleAlt) {
            ...
            if (blk.getAlternativeAt(0).synPred != null) {
                ...
                println(
                  "Warning: you specified a syntactic predicate for this alternative,");
                println(
                  "and it is the only alternative of a block and will be ignored.");
            }
        } else {
    ...
    public void genRule(RuleSymbol s) {
        ...
        if (!s.isDefined()) {
            ...
            println(
              "This means that the rule was referenced somewhere in the grammar,");
            println("but a definition for the rule was not encountered.");
            println(
              "It is also possible that syntax errors during the parse of");
            println(
              "your grammar file prevented correct processing of the rule.");
        ...
        if (rblk.returnAction != null) {
            ...
            if (doingLexRules) {
                println(
                  "Error: you specified return value(s) for a lexical rule.");
                println(
                  "\tLexical rules have an implicit return type of 'int'.");
            }
        } else {
            if (doingLexRules) {
                println(
