 ...
 * or e-sent to any methodical object.
 * <p/>
...
public class MirandaMethods {
     ...
     * to certain clients by implementing {@link #__optSealedDispatch} as
     * described there.
     * <p/>
     ...
     * object, according to
     * {@link org.erights.e.elib.ref.Ref#isSameEver E's "==" operation}. The
     * Selfless auditor ensures that all Selfless objects are
     ...
     */
    static public void __reactToLostClient(Object self, Throwable problem) {
        T.noop();
    }
     ...
     *
     * @see org.erights.e.elib.ref.Ref#whenBroken(Object,
      *      org.erights.e.elib.util.OneArgFunc) Ref.whenBroken/2
     */
     ...
     * <p/>
     * '<tt>foo::bar</tt>' expands to
     * '<tt>foo.__getPropertySlot("bar").getValue()</tt>'.
     * When used an an lValue, '<tt>foo::bar := newValue</tt>' expands
     * essentially to
     * '<tt>foo.__getPropertySlot("bar").setValue(newValue)</tt>'
     * except that the expansion also has the value of the assignment
     * expression be the value assigned.
     * <p>
     * And finally
     * '<tt>foo::&bar</tt>' expands to '<tt>foo.__getPropertySlot("bar")</tt>'
     * <p><hr>
     * The Miranda behavior provided here synthesizes, for foo's bar property,
     * a Slot object <ul>
     * <li>whose '<tt>getValue()</tt>' does a '<tt>foo.getBar()</tt>'
     * <li>whose '<tt>setValue(newValue)</tt>' does a
     *     '<tt>foo.setBar(newValue)</tt>'
     * <li>and whose '<tt>isFinal()</tt>' returns false</tt>'.
     * </ul>
     *
     * @see <a href=
     * "http://www.eros-os.org/pipermail/e-lang/2004-April/009720.html"
     ...
     * {@link #__optUncall} would otherwise have returned. By so doing, the
     * object reveals its internals only to someone having the corresponding
     * Unsealer.
     * <p/>
     * If you know self to be a {@link org.erights.e.elib.sealing.Amplifiable},
     * you can invoke this more efficiently from Java by
     * <pre>    self.__optSealedDispatch(brand);</pre> Else by
     * <pre>    Ref.optSealedDispatch(self, brand);</pre>
     */
    static public SealedBox __optSealedDispatch(Object self, Brand brand) {
        return null;
    }

    /**
     * When a guard doesn't succeed by itself at coercing an object,
     * the guard may enlists the object's aid in bring about a coercion
     ...
     * A Gozarian guard wants to enlists the object's aid <i>because</i>
     ...
     * MirandaMethod.
     * <p/>
     * The guard asks the object to conform to some guard that the
     * object may know about. Often this will be the guard that's
     * attempting the coercion. An object should attempt to return a
     * representation of itself that the argument guard would succeed in
     * coercing. A requesting guard should then try again on the result,
    ...
    /**
     * For E, this is the method that is overridden, rather
     * than toString(), since composition via toString() cannot break
     * cycles.
     * <p/>
     * If you know self to be a {@link EPrintable},
     * you can invoke this more efficiently from Java by
      ...
      throws IOException {
        ...
        if (null == self) {
            ...
            if (toStringMethod.getDeclaringClass() == Object.class) {
                ...
                out.write("<" + StringHelper.aan(ClassDesc.simpleSig(clazz)) +
                          ">");
            } else {
