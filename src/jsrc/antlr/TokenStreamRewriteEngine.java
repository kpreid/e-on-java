 ...
 * does not pass on the whitespace (or whatever else you want to discard) to
 * the parser.
 * <p/>
 ...
 * efficient because you are not moving data around all the time.  As the
 * buffer of tokens is converted to strings, the toString() method(s) check to
 * see if there is an operation at the current index.  If so, the operation is
 * done and then normal String rendering continues on the buffer.  This is like
 * having multiple Turing machine instruction streams (programs) operating on a
 * single input tape. :)
 * <p/>
 ...
 * are queued up, you can easily simulate transactions and roll back any
 * changes if there is an error just by removing instructions. For example,
 * <p/>
 ...
 * rewriteEngine.insertAfter("pass1", t, "foobar");} rewriteEngine.insertAfter("pass2",
 * u, "start");} System.out.println(rewriteEngine.toString("pass1"));
 * System.out.println(rewriteEngine.toString("pass2"));
 * <p/>
 * You can also have multiple "instruction streams" and get multiple rewrites
 * from a single pass over the input.  Just name the instruction streams and
 * use that name again when printing the buffer.  This could be useful for
 * generating a C file and also its header file--all from the same buffer.
 * <p/>
 * If you don't use named rewrite streams, a "default" stream is used.
 * <p/>
 * Terence Parr, parrt@cs.usfca.edu University of San Francisco February 2004
 */
public class TokenStreamRewriteEngine implements TokenStream {
    ...
    public void rollback(String programName, int instructionIndex) {
        ...
        if (is != null) {
            programs.put(programName,
                         is.subList(MIN_TOKEN_INDEX, instructionIndex));
        }
    }
    ...
    /**
     * If op.index > lastRewriteTokenIndexes, just add to the end. Otherwise,
     * do linear
     */
