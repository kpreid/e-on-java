...
public class TestELib {
    ...
    /**
     * TestClass -- inner class that has the method we'll be using
     * as a message. (Note that this doesn't need to be an inner class.)
     * <p/>
     * To run it: java org.erights.e.elib.tests.TestELib
     * Output should be:
     * Adding one...
     * Adding one to 5
     * result is 6
    ...
    public class TestClass {
        ...
        /**
         * addOne -- takes an int32 value and adds one to it, then returns
         * the new value.
         */
        public Number addOne(Integer val) throws Exception {
            PrintStreamWriter.stdout().println("addOne, val is " +
                                               val.intValue() + "... ");
            if (val.intValue() == 5) {
                PrintStreamWriter.stdout().println(
                  "addOne... throwing exception");
    ...
    public void testit(int startVal) {
        ...
        Object p = E.send(target,
                          "addOne",
                          EInt.valueOf(startVal));
        ...
        OneArgFuncAdapter.adapt(p, "__whenMoreResolved",
                                this, "reactToAddOneResolved");
