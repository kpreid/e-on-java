...
public class Tool {
    ...
    protected void checkForInvalidArguments(String[] args,
        ...
        for (int a = 0; a < args.length; a++) {
            if (!cmdLineArgValid.member(a)) {
                warning(
                  "invalid command-line argument: " + args[a] + "; ignored");
            }
        }
    }
    ...
    public int doEverything(String[] args) {
        ...
        TokenBuffer tokenBuf = new TokenMultiBuffer(lexer);
     ...
     *               (or -1)
     * @param column The grammar file column number on which the error occured
     *               (or -1)
     */
     ...
     * } c = Class.forName (p); Constructor con = c.getConstructor (cl); o =
     * con.newInstance (initargs); } catch (Exception e) { System.err.println
     * ("Can't make a " + p); } return o; }
     */
     ...
     * java.lang.RuntimeException}. <em>If this method is overriden, <strong>it
    ...
    /**
     * Issue a fatal error message. <em>If this method is overriden, <strong>it
     * must never return normally</strong>; i.e. it must always throw an
     * exception or call System.exit</em>.
     *
     * @param s The message
    ...
    /**
     * <p>Process the command-line arguments.  Can only be called by Tool. A
     * bitset is collected of all correct arguments via setArgOk.</p>
     *
     ...
     *               (or -1)
     * @param column The grammar file line number on which the warning occured
     *               (or -1)
     */
    ...
    public void setNameSpace(String name) {
        if (null == nameSpace) {
            nameSpace =
              new NameSpace(StringUtils.stripFrontBack(name, "\"", "\""));
        }
    }
}
