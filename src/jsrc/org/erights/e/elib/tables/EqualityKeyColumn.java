...
class EqualityKeyColumn extends KeyColumn {
     ...
     * @param capacity
     */
    public EqualityKeyColumn(Class memberType, int capacity) {
    ...
    int findPosOf(Object key) {
        int hash = 0;
        if (key != null) {
            hash = key.hashCode() & POSITIVE_MASK;
        }
    ...
    int store(Object key) {
        ...
        for (; ;) {
            ...
            } else if (hash == myHashes[curPos] && curKey.equals(key)) {
                // we found it.
                return curPos;
            }
            curPos += probeSkip;
