...
public class PreservingFileWriter extends FileWriter {
        ...
        if (parentdirname != null) {
            ...
            if (!parentdir.exists()) {
                throw new IOException(
                  "destination directory of '" + file + "' doesn't exist");
            }
            if (!parentdir.canWrite()) {
                throw new IOException(
                  "destination directory of '" + file + "' isn't writeable");
            }
        }
    ...
    /**
     * Close the file and see if the actual target is different if so the
     * target file is overwritten by the copy. If not we do nothing
     */
