...
public class MakeGrammar extends DefineGrammarSymbols {
    ...
    public void beginTree(Token tok) throws SemanticException {
        if (!(grammar instanceof TreeWalkerGrammar)) {
            tool.error("Trees only allowed in TreeParser",
                       grammar.getFilename(),
                       tok.getLine(),
                       tok.getColumn());
        ...
        for (int i = 0; i < lexRules.size(); i++) {
            ...
            if (!r.isDefined()) {
                ...
                if ("public".equals(r.access)) {
                    ...
                    RuleRefElement rr = new RuleRefElement(g,
                                                           new CommonToken(
                                                             ANTLRTokenTypes.RULE_REF,
                                                             r.getId()),
    ...
    public void endExceptionSpec() {
        ...
        if (currentExceptionSpec == null) {
            tool.fatalError(
              "exception processing internal error -- no active exception spec");
        }
        if (context().block instanceof RuleBlock) {
            // Named rule
    ...
    public void oneOrMoreSubRule() {
        if (context().block.not) {
            tool.error("'~' cannot be applied to (...)* subrule",
                       grammar.getFilename(),
                       context().block.getLine(),
                       context().block.getColumn());
        }
    ...
    public void refCharLiteral(Token lit,
                               ...
                               boolean lastInRule) {
        if (!(grammar instanceof LexerGrammar)) {
            tool.error("Character literal only valid in lexer",
                       grammar.getFilename(),
                       lit.getLine(),
                       lit.getColumn());
            return;
    ...
    public void refCharRange(Token t1,
                             ...
                             boolean lastInRule) {
        if (!(grammar instanceof LexerGrammar)) {
            tool.error("Character range only valid in lexer",
                       grammar.getFilename(),
                       t1.getLine(),
                       t1.getColumn());
            return;
    ...
    public void refToken(Token idAssign,
        ...
        if (grammar instanceof LexerGrammar) {
            ...
            if (inverted) {
                tool.error("~TOKEN is not allowed in lexer",
                           grammar.getFilename(),
                           t.getLine(),
                           t.getColumn());
            }
            ...
            if (args != null) {
                tool.error("Token reference arguments only allowed in lexer",
                           grammar.getFilename(),
                           args.getLine(),
                           args.getColumn());
            }
            super.refToken(idAssign,
                           t,
                           label,
                           args,
                           inverted,
    ...
    public void refTokenRange(Token t1,
                              ...
                              boolean lastInRule) {
        if (grammar instanceof LexerGrammar) {
            tool.error("Token range not allowed in lexer",
                       grammar.getFilename(),
                       t1.getLine(),
                       t1.getColumn());
            return;
    ...
    public void synPred() {
        if (context().block.not) {
            tool.error("'~' cannot be applied to syntactic predicate",
                       grammar.getFilename(),
                       context().block.getLine(),
                       context().block.getColumn());
        }
