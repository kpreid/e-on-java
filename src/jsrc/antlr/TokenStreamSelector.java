 ...
 * parser.  This is a way to have multiple lexers break up the same input
 * stream for a single parser. Or, you can have multiple instances of the same
 * lexer handle multiple input streams; this works great for includes.
 */
public class TokenStreamSelector implements TokenStream {
    ...
    /**
     * Abort recognition of current Token and try again. A stream can push a
     * new stream (for include files for example, and then retry(), which will
     * cause the current stream to abort back to this.nextToken().
     * this.nextToken() then asks for a token from the current stream, which is
     * the new "substream."
     */
