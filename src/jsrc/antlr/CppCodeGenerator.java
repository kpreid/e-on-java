...
public class CppCodeGenerator extends CodeGenerator {
     ...
     * still call setTool, setBehavior, and setAnalyzer before generating
     * code.
     */
     ...
     * debugging parser.  This method should only be called when the debug
     * option is specified
     */
    ...
    public void printAction(Token t) {
        if (t != null) {
            genLineNo(t.getLine());
            printTabs();
            _printAction(processActionForSpecialSymbols(t.getText(),
                                                        t.getLine(),
                                                        null,
                                                        null));
            genLineNo2();
    ...
    public void printHeaderAction(String name) {
        ...
        if (a != null) {
            genLineNo(a.getLine());
            println(processActionForSpecialSymbols(a.getText(),
                                                   a.getLine(),
                                                   null,
                                                   null));
            genLineNo2();
     ...
     * multibyte (bit crude). Then reescape to C++ style things. Used to
     * generate strings for match() and matchRange()
     *
     * @param lit           the literal string
     * @param isCharLiteral if it's for a character literal (enforced to be one
     *                      length) and enclosed in ' FIXME: bombing out on mb
     *                      chars. Should be done in Lexer. FIXME: this is
     *                      another horrible hack. FIXME: life would be soooo
     *                      much easier if the stuff from the lexer was
                ...
                case'u':
                    ...
                    if (i + 5 < s.length()) {
                        val = Character.digit(s.charAt(i + 2), 16) * 16 * 16 *
                          16 + Character.digit(s.charAt(i + 3), 16) * 16 * 16 +
        ...
        try {
            ...
            while (grammarIter.hasMoreElements()) {
                ...
                if (g.debuggingOutput) {
                    antlrTool.error(
                      g.getFilename() + ": C++ mode does not support -debug");
                }
    ...
    public void gen(CharLiteralElement atom) {
        ...
        if (!(grammar instanceof LexerGrammar)) {
            antlrTool.error(
              "cannot ref character literals in grammar: " + atom);
        }
...
//        }
        ...
        if (0 == syntacticPredLevel) {
            ...
            if (genAST) {
                ...
                case GrammarElement.AUTO_GEN_CARET:
                    ...
                    antlrTool.error(
                      "Internal: encountered ^ after rule reference");
                    break;
                default:
                    break;
                }
     ...
     *                      least this value.
     * @param prefix        string glued in from of bitset names used for
...
//		println("setCaseSensitive("+g.caseSensitive+");");
        ...
        while (keys.hasMoreElements()) {
            ...
            if (sym instanceof StringLiteralSymbol) {
                ...
                println(
                  "literals[" + s.getId() + "] = " + s.getTokenType() + ";");
            }
        }
        ...
        if (grammar.debuggingOutput) {
            println(
              "const char* " + grammar.getClassName() + "::_ruleNames[] = {");
            tabs++;
...
//		println("setTokenNames(_tokenNames);");
     ...
     *
     * @param blk             The block to generate
     * @param noTestForSingle If true, then it does not generate a test for a
     *                        single alternative.
     */
...
//					println(labeledElementASTType+" " + astName + " = "+labeledElementASTInit+";");
    ...
    /**
     * Generate a try { opening if the element has a labeled handler in the
     * rule
     */
    ...
    public void genInclude(LexerGrammar g) throws IOException {
        ...
        // Generate lexer class definition
        print(
          "class CUSTOM_API " + grammar.getClassName() + " : public " + sup);
        println(
          ", public " + grammar.tokenManager.getName() + TokenTypesFileSuffix);
...
//		println("void _initialize( void );");
        ...
        println("enum {");
        println(
          "\tNUM_TOKENS = " + grammar.tokenManager.getVocabulary().size());
        println("};");
        _println("#endif");
    ...
    public void genNextToken() {
        ...
        if (((LexerGrammar)grammar).filterMode) {
            if (filterRule == null) {
                ...
                println(
                  "catch(" + namespaceAntlr + "RecognitionException& ee) {");
...
//		}
     ...
     * the token type values.  This static array is used to format error
     * messages so that the token identifers or literal strings are displayed
     * instead of the token numbers.
     * <p/>
...
//		println("");
    ...
    protected void genTokenTypes(TokenManager tm) throws IOException {
        ...
        // Generate a guard wrapper
        println(
          "#ifndef INC_" + tm.getName() + TokenTypesFileSuffix + "_hpp_");
        println(
          "#define INC_" + tm.getName() + TokenTypesFileSuffix + "_hpp_");
        println("");
        ...
        println(
          "struct CUSTOM_API " + tm.getName() + TokenTypesFileSuffix + " {");
        println("#endif");
        tabs++;
        println("enum {");
        tabs++;
        ...
        // Generate a guard wrapper
        println(
          "#endif /*INC_" + tm.getName() + TokenTypesFileSuffix + "_hpp_*/");
     ...
     * used to cast simple tokens/references to the right type for the
     * generated language. Basically called for every element in the vector to
...
//			System.out.println("processStringForASTConstructor: "+str);
     ...
     *
     * @param v A Vector of String, where each element is an expression in the
        ...
        if (atom != null && atom.getASTNodeType() != null) {
            ...
            if (type == null) {
                ...
                if (!atom.getASTNodeType().equals(type)) {
                    antlrTool.warning(
                      "Attempt to redefine AST type for " + atom.getText(),
                      grammar.getFilename(),
                      atom.getLine(),
                      atom.getColumn());
                    ...
                    astTypes.setElementAt(atom.getASTNodeType(),
                                          atom.getType());
                }
            }
...
//			System.out.println("getAstCreateString(as): "+str+" "+grammar.tokenManager.tokenDefined(str));
            if (usingCustomAST && (grammar instanceof TreeWalkerGrammar) &&
              !(grammar.tokenManager.tokenDefined(str)) && !is_constructor) {
                return "astFactory->create(" + namespaceAntlr + "RefAST(" +
                  str + "))";
            } else {
                return "astFactory->create(" + str + ")";
            }
...
*/
     ...
     *
     * @param k     The lookahead level
     * @param elems The elements representing the set, usually from
     *              BitSet.toArray().
     ...
     * context-sensitive, depending on the rule and alternative being
     * generated
     *
        ...
        if (g instanceof ParserGrammar) {
            ...
            if (!g.hasOption("ASTLabelType")) {
                g.setOption("ASTLabelType",
                            new Token(ANTLRTokenTypes.STRING_LITERAL,
                                      namespaceAntlr + "RefAST"));
            }
            commonExtraArgs = "_t";
            commonLocalVars = "";
            exceptionThrown = namespaceAntlr + "RecognitionException";
