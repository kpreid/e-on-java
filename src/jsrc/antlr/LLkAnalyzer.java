...
//            inner:
     ...
     * <p/>
     * if we hit the end of a loop, we have to include what tokens can begin
     * the loop as well.  If the start node is locked, then we simply found an
     * empty path through this subrule while analyzing it.  If the start node
     * is not locked, then this node was hit during a FOLLOW operation and the
     * FIRST of this block must be included in that lookahead computation.
     */
    ...
    /**
     * Return this char as the lookahead if k=1. <p>### Doesn't work for ( 'a'
     * 'b' | 'a' ~'b' ) yet!!!
     * <p/>
     * If the atom has the <tt>not</tt> flag on, then create the set complement
     * of the tokenType which is the set of all characters referenced in the
     * grammar with this char turned off. Also remove characters from the set
     * that are currently allocated for predicting previous alternatives.  This
     * avoids ambiguity messages and is more properly what is meant. ( 'a' |
     ...
     * particular ref to the node.  In other words, we use context information
     * to reduce the complexity of the analysis and strengthen the parser.
     * <p/>
    ...
    public Lookahead look(int k, RuleRefElement rr) {
        ...
        if (rs == null || !rs.defined) {
            tool.error("no definition of rule " + rr.targetRule,
                       grammar.getFilename(),
                       rr.getLine(),
                       rr.getColumn());
            return new Lookahead();
     ...
     * analysis (you want to try it out at run-time).  The LOOK of (a)=>A B is
     * A for LL(1) ### is this even called?
     */
    ...
    /**
     * The (...)* element is the combined lookahead of the alternatives and
     * what can follow the loop.
     */
