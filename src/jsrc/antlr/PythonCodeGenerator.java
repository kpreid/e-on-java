...
public class PythonCodeGenerator extends CodeGenerator {
    ...
    /**
     * Get the identifier portion of an argument-action. For Python the ID of
     * an action is assumed to be everything before the assignment, as Python
     * does not support a type.
     *
    ...
    public void gen(LexerGrammar g) throws IOException {
        ...
        println(
          "self.caseSensitiveLiterals = " + toString(g.caseSensitiveLiterals));
    ...
    public void gen(WildcardElement wc) {
        ...
        if (grammar instanceof TreeWalkerGrammar) {
            ...
            println(
              "self.matchNot(" + getValueString(Token.EOF_TYPE, false) + ")");
        }
...
//  }
    ...
    private void genLiteralsTestForPartialToken() {
        println(
          "_ttype = self.testLiteralsTable(self.text.getBuffer(), _begin, self.text.length()-_begin), _ttype)");
    }
...
// mx
                println(extractIdOfAction(rblk.returnAction,
                                          rblk.getLine(),
                                          rblk.getColumn()) + " = None");
            }
        }
...
//	    }
    ...
    /**
     * Get a string for an expression to generate creating of an AST node.
     * Parse the first (possibly only) argument looking for the token type. If
     * the token type is a valid token symbol, ask for it's AST node type and
