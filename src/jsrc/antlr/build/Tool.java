...
/**
 * An application-specific build tool for ANTLR.  Old build was a shell script
 * that of course is not portable.  All it did though was to compile; this Java
 * program is a portable compile script. :)
 * <p/>
 * This class knows how to portable invoke a system command with wildcard
 * expansion.  It also knows to how dynamically load a project description
 * based upon the name.  Actions are just method names within that object.  The
 * ANTLR object defines build and jar methods that use Tool to portably compile
 * and jar--it is the replacement the script.
 * <p/>
 * I almost used Python for this, but I hate forcing users to download yet
 * another tool just to compile this one.  (Another reason not to use Ant).
 * <p/>
 * I am using a class to describe the build (e.g., ANTLR) rather than a file
 * format at the moment since I am not sure how all this will progress.
 * <p/>
 * Properties antlr.build.compiler and antlr.build.root may be set according to
 * your needs.  They default to "javac" and ".".  The root is the dir
 * containing the antlr directory associated with the antlr Java package.
 * <p/>
 ...
 * <p/>
 * from the directory containing package antlr's "antlr" dir.  Or, use
 * something like:
 * <p/>
...
public class Tool {
    ...
    /**
     * Find a class named after the app and then find a method within that is
    ...
    /**
     * A portable system command execution routine that logs stdout/stderr and
     * handles cmd-line wildcards properly.
     */
