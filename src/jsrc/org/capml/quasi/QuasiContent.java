 ...
 * Nodes. The latter we also define as a "ContentList". As a ValueMaker, a
 * QuasiContent will evaluate to a ContentList. As a MatchMaker, it will match
 * against Content, and either succeed, extracting values, or fail. The
 * Minimal-XML quasi-parser produces a QuasiContent.
 *
...
public abstract class QuasiContent
  implements Persistent, Selfless, JOSSPassByConstruction, MatchMaker {
     ...
     * returned. If it's a String, a singleton list of a Text on that String is
     * returned. Otherwise, content must be a ConstList of Nodes, and is
     * returned.
     */
     ...
     * <p/>
     * If it's a String, return the corresponding Text. Otherwise return null.
     * If content isn't valid Content, it's ok to throw an exception instead of
     * returning null.
     */
     ...
     * <p/>
     * If 'content' is a Node, it's returned. If it's a String, a Text on that
     * String is returned. Otherwise, it must be a ConstList of Nodes. If it's
     * a singleton list, it's one member is returned. Otherwise the list is
     * returned.
     */
    ...
    /**
     * A convenience for placing @-hole matches into bindings. XXX Note that it
     * will not detect multiple stores of null to the same index.
     */
     ...
     * <p/>
     * The args and return value must be concrete (not quasi). If it's filling
     * in an Element tagName, the arg must be a String. Otherwise it may be
     ...
     * match against the specimen, which must be Content or String, and return
     * the list of values extracted by @-holes.
     * <p/>
     ...
     * <p/>
     * The args, specimen, and the values put into bindings must be concrete.
     * The args must follow the same rules as the args to substitute(). The
     * specimen may be either Content or String. The values extracted by
     * @-holes are Strings, if they were extracted by a tagName @-hole, or a
     * Content, if they were extracted by an @-hole in content position.
     */
    ...
    /**
     * Prints as "sml`...`", where pretty printed quasi-XML appears between the
     * backquotes
     */
