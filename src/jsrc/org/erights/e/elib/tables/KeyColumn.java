...
abstract class KeyColumn extends Column {
    ...
    /**
     * The sparse array of keys by position. null is a valid entry. The value
     * of an entry is only valid if indicated by the corresponding myPos2Rank
     * entry.
     */
    ...
    /**
     * A sparse array parallel to myKeys. Each element contains either
     * KEY_UNUSED, KEY_DELETED, or an index into myRank2Pos indicating where
     * the association at this position fits into the ordering.
     */
    final int[] myPos2Rank;

    /**
     * A value in myPos2Rank indicating that this position has never been
     * occupied since the last rehash. Note that KEY_UNUSED is negative, so it
     * can't conflict with an array index.
     */
    static final int KEY_UNUSED = -1;

    /**
     * A value in myPos2Rank indicating that this position used to be occupied
     * since the last rehash, but no longer is. Note that KEY_DELETED is
     ...
     * <p/>
     * The allocated size of myRank2Pos is actually the same as the sparse
     * arrays, but it's dense in 0..!myNumTaken, and unused after that.
     */
     ...
     *
     * @param memberType All keys will be (implicitly) checked for conformance
     *                   to this type.
     * @param capacity   The column will hold at least this number of
     *                   positions. To find out the actual number, call
     *                   capacity() on the constructed column.
     */
    ...
    /**
     * Makes a key column that's equivalent to a SamenessKeyColumn(memberType,
     * capacity), but may select a more efficient implementation based on how
     * instances of memberType compare for sameness.
     */
    ...
    /**
     * Returns the pos at which key resides, or -1 if the key is absent from
     * the map.
     */
    abstract int findPosOf(Object key);

    /**
     * Returns the first good size for a key column that's no less than
     * candidate. This will be a prime number so that we get better
     * distribution of elements through the map.
     */
    ...
    /**
     * Put the given key into the map, and return its pos. If the key is
     * equivalent (according to the concrete column's equality function) to a
     * key already in the map, that pos is returned. If the key is novel but
     * the map is too small to add it, a -1 is returned. <p>
     * <p/>
     * If the key already exists, no ordering information is affected. If a
     * novel key is inserted, it's position gets the next rank.
     *
     * @param key the key to place in the map
     * @return the pos at which the key now resides in the map, or -1 if we
     *         need more room.
     */
