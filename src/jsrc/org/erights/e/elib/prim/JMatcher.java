...
public interface JMatcher {
    ...
    /**
     * When a JMatcher (or any object with a truly public
     * 'match(String, ConstList)' method) is invoked through ELib, if no
     * matching method is found, then the match/2 method is invoked with the
     * verb (message name) and args of the message that didn't match.
     * <p/>
     * When defining a matcher, it's a good idea to define it to respond to
     * {@link MirandaMethods#__respondsTo} and
     * {@link MirandaMethods#__getAllegedType} appropriately. Most appropriate
     * would be to respond with a description of the behavior of the matcher
     * as a whole. But a degenerate behavior is better than none.
     * <p/>
     * XXX To solve bug <a href=
