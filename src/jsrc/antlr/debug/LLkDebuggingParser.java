...
public class LLkDebuggingParser extends LLkParser implements DebuggingParser {
    ...
    protected boolean fireSemanticPredicateEvaluated(int type,
                                                     ...
                                                     boolean condition) {
        if (isDebugMode()) {
            return parserEventSupport.fireSemanticPredicateEvaluated(type,
                                                                     num,
                                                                     condition,
                                                                     inputState.guessing);
        } else {
     ...
     * current token being examined by the parser (i.e., it has not been
     * matched yet).
     */
    ...
    /**
     * Make sure current lookahead symbol matches token type <tt>t</tt>. Throw
    ...
    public void matchNot(int t)
      throws MismatchedTokenException, TokenStreamException {
        ...
        try {
            super.matchNot(t);
            parserEventSupport.fireMatchNot(la_1,
                                            t,
                                            text,
                                            inputState.guessing);
        } catch (MismatchedTokenException e) {
