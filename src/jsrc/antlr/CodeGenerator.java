...
/**
 * A generic ANTLR code generator.  All code generators Derive from this
 * class.
 * <p/>
 * <p/>
...
public abstract class CodeGenerator {
     ...
     * equal to this number of non-predicated LL(1) alternates. This is
     * modified by the grammar option "codeGenMakeSwitchThreshold"
     */
     ...
     * comparisions for lookahead sets with degree greater than or equal to
     * this value. This is modified by the grammar option
     ...
     *
     * @param elems The array of elements representing the set, usually from
     *              BitSet.toArray().
     ...
     * may want to override this if the language has unusual declaration
     * syntax.
     *
     ...
     * assumed to be a trailing identifier. Specific code-generators may want
     * to override this if the language has unusual declaration syntax.
     *
    ...
    protected String extractIdOfAction(String s, int line, int column) {
        ...
        antlrTool.warning("Ill-formed action",
                          grammar.getFilename(),
                          line,
                          column);
        return "";
     ...
     * @param forInput true if the input tree node variable is to be returned,
     ...
     * end of the list, so the caller can rely on the position of bitsets in
     * the list. The returned position can be used to format the bitset name,
     * since it is invariant.
     *
     * @param p         Bit set to mark for code generation
     * @param forParser true if the bitset is used for the parser, false for
     *                  the lexer
    ...
    /**
     * Lexically process $ and # references within the action. This will
    ...
    protected void setGrammar(Grammar g) {
        ...
        if (grammar.hasOption("codeGenMakeSwitchThreshold")) {
            try {
                makeSwitchThreshold =
                  grammar.getIntegerOption("codeGenMakeSwitchThreshold");
        ...
        if (grammar.hasOption("codeGenBitsetTestThreshold")) {
            try {
                bitsetTestThreshold =
                  grammar.getIntegerOption("codeGenBitsetTestThreshold");
